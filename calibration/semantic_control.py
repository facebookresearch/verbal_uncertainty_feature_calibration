...
def load_detection_res(dataset, model_name, split):
    path = f"{root_path}/detection/LR_outputs/{dataset}/{model_name}/{split}_verbal_uncertainty_sentence_semantic_entropy.json"
    with open(path) as f:
...
if __name__ == '__main__':
    ...
    detection_res = load_detection_res(dataset, model_name, split)
    ...
    if args.iti_method == 0:
        ...
        print('alphas', alphas)
        verbal_uncertain_feats = torch.load(f'{root_path}/calibration/outputs/merged/{model_name}/{args.prompt_type}/Hs_hedge_universal.pt')
    elif args.iti_method == 3: # radom feature
        seed = args.seed
        out_file = f"{output_dir}/with_vufi_{args.iti_method}{seed}_{args.str_process_layers}_{args.max_alpha}.jsonl"
        alphas = (su_scores/MAX_SE - vu_scores_llm) * MAX_ALPHA
        # max alphas and 0
        alphas = np.clip(alphas, 0, MAX_ALPHA)
        alphas = np.round(alphas, 4)
        print('alphas', alphas)
        verbal_uncertain_feats = torch.load(f'{root_path}/calibration/outputs/merged/{model_name}/{args.prompt_type}/Hs_hedge_universal.pt')
        max_item, min_item = torch.max(verbal_uncertain_feats), torch.min(verbal_uncertain_feats)
        torch.manual_seed(seed)
        verbal_uncertain_feats = torch.randn(verbal_uncertain_feats.shape) * (max_item - min_item) + min_item
    else:
        assert False
